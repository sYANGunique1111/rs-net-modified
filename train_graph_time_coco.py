...
def step(split, opt, dataLoader, model, criterion, optimizer=None,logger=None, lr = 0):#, key_index=None
    ...
    for i, data in enumerate(tqdm(dataLoader, 0)):
        ...
        if opt.test_augmentation and split =='test':
            input_2D, output_3D = input_augmentation(input_2D, model_rsnet, joints_left, joints_right)
        else:
            input_2D = input_2D.view(N, -1, opt.n_joints,opt.in_channels, 1).permute(0, 3, 1, 2, 4).type(torch.cuda.FloatTensor) # N, C, T, J, M
            output_3D = model_rsnet(input_2D)
        ...
        #calculate loss 
        loss_gt = (1-opt.lamda)*criterion_mse(pred_out, out_target) + opt.lamda*criterion_L1(pred_out, out_target)
...
def input_augmentation(input_2D, model_rsnet, joints_left, joints_right):
    ...
    output_3D_flip = model_rsnet(input_2D_flip)
    output_3D_flip[:, 0] *= -1
    output_3D_flip[:, :, :, joints_left + joints_right] = output_3D_flip[:, :, :, joints_right + joints_left]
    output_3D_non_flip = model_rsnet(input_2D_non_flip)
